    ...
    angular.module('angucomplete-alt', []).directive('angucompleteAlt', [
        ...
        function(
            ...
            function link(scope, elem, attrs, ctrl) {
                ...
                unbindInitialValue = scope.$watch('initialValue', function(
                    newval
                ) {
                    if (newval) {
                ...
                scope.$watch('fieldRequired', function(newval, oldval) {
                    if (newval !== oldval) {
                        if (!newval) {
                            ctrl[scope.inputName].$setValidity(
                                requiredClassName,
                                true
                            );
                ...
                scope.$on('angucomplete-alt:changeInput', function(
                    event,
                    elementId,
                    newval
                ) {
                ...
                function dropdownRow() {
                    return elem[0].querySelectorAll('.angucomplete-row')[
                        scope.currentIndex
                    ];
                }
                ...
                function keydownHandler(event) {
                    ...
                    if (which === KEY_EN && scope.results) {
                        if (
                            ...
                            event.preventDefault();
                            scope.selectResult(
                                scope.results[scope.currentIndex]
                            );
                        } else {
                ...
                function getRemoteResultsWithCustomHandler(str) {
                    ...
                    scope
                        .remoteApiHandler(str, httpCanceller.promise)
                        .then(httpSuccessCallbackGen(str))
                        .catch(httpErrorCallback);
        ...
        */
                    ...
                    if (responseData && responseData.length > 0) {
                        ...
                        for (i = 0; i < responseData.length; i++) {
                            if (scope.titleField && scope.titleField !== '') {
                                text = formattedText = extractTitle(
                                    responseData[i]
                                );
                            }
                ...
                scope.onFocusHandler = function() {
                    ...
                    if (
                        minlength === 0 &&
                        (!scope.searchStr || scope.searchStr.length === 0)
                    ) {
                        scope.currentIndex = scope.focusFirst
                            ? 0
                            : scope.currentIndex;
                        scope.showDropdown = true;
                        showAll();
                ...
                scope.textSearching = attrs.textSearching
                    ? attrs.textSearching
                    : TEXT_SEARCHING;
                scope.textNoResults = attrs.textNoResults
                    ? attrs.textNoResults
                    : TEXT_NORESULTS;
                displaySearching =
                    scope.textSearching === 'false' ? false : true;
                displayNoResults =
                    scope.textNoResults === 'false' ? false : true;
                ...
                responseFormatter = callFunctionOrIdentity(
